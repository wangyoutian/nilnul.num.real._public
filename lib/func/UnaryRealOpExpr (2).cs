using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;




namespace nilnul.realNumber
{
	public partial class UnaryRealOpExpr
		:
		UnaryOpExprA<ExprI<RealI>>
		,
		UnaryRealOpExprI
	{


		public UnaryOpI<ExprI<RealI>, ExprI<RealI>> op
		{
			get;
			set;
		}

		public UnaryRealOpExpr(UnaryOpI<ExprI<RealI>, ExprI<RealI>> op, RealI operand)
			:base(operand)
		{
			this.op = op;
			
		}



		#region RealI Members

		public ExprI<RealI> simplify()
		{
			throw new NotImplementedException();
		}

		#endregion

		#region IComparable Members

		public int CompareTo(object obj)
		{
			throw new NotImplementedException();
		}

		#endregion

		#region IComparable<ExtendedRealI> Members

		public int CompareTo(ExtendedRealI other)
		{
			throw new NotImplementedException();
		}

		#endregion
	}
}
